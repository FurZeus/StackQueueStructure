...
1. **Stack -> Queue:** Stack boşalana kadar elemanlar çıkarılır (`pop`) ve kuyruğa eklenir (`enqueue`).
2. **Queue -> Stack:** Kuyruk boşalana kadar elemanlar çıkarılır (`dequeue`) ve tekrar stack'e eklenir (`push`).
...
1. Bu repoyu bilgisayarınıza indirin (veya `git clone` yapın).
2. `index.html` dosyasına çift tıklayarak tarayıcınızda açın.
3. **"Başlat"** butonuna basarak simülasyonu izleyin.
...
2. Sol menüden **Pages**'i seçin.
3. **Branch** kısmından `main` (veya master) seçip **Save**'e tıklayın.
4. GitHub size canlı bir link verecektir (örn: `https://kullaniciadi.github.io/repo-adi/`).
